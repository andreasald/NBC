...
{
    public partial class inputPemeriksaan : Form
    {

        System.Windows.Forms.Form f = System.Windows.Forms.Application.OpenForms["Pemeriksaan"];

        public inputPemeriksaan()
        {
            InitializeComponent();
            translateHari();
            ShiftByNow();
        ...
        private void PendaftaranTransaksi_Load(object sender, EventArgs e)
        {
            ...
            pilihPerawatan1.setFlag(0);
            pilihProduk1.setFlag(0);
            //isiData(idTransaksi.Text);

            cekTanggal();
            idTransaksi.Text = ((Pemeriksaan)f).idTransaksinya.Text;
            setDataGridView(this.dataGridView1);
            setDataGridView2(this.dataGridView2);

            string jkCus = TSC_C.getJenisKelCus(idTransaksi.Text);
        ...
        private void btnSimpan_Click(object sender, EventArgs e)
        {
            if (cek() == true)
            {
                ...
                string idCustomer = TSC_C.getIDPelanggan(idTransaksi.Text);
                
                iNBC.Entity.Transaksi T = new Entity.Transaksi(idTransaksi.Text, idCustomer, int.Parse(cmbRuang.Text), "-", IDofCurrentShift, System.DateTime.Now, txtKeluhan.Text, 0, "Receivable", 1);
